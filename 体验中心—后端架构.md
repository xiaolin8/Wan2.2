...
spec:
  ...
  - matches:
    - path:
        type: PathPrefix
        value: /v1/video/generations/tasks
      headers:
      - name: plugin
        type: Exact
        value: true
  ...
  - matches:
    - path:
        type: PathPrefix
        value: /v1/video/generations/tasks # 使用路径前缀匹配，以支持 /{task_id} 这样的 URL
      headers:
      - name: plugin
        type: Exact
        value: true
...
curl --location 'https://<GATEWAY-IP>/v1/video/generations' \
    ...
    "input": {
        "prompt": "一只猫在草地上奔跑",
        "img_url": "data:image/png;base64,aHR0******cG5n"
    },
    "parameters": {
        "resolution": "720P",
        "prompt_extend": false
...
- **img_url** `*string*` **（必选）**
  ...
  - **resolution** `*string*` （可选）
    ...
    - 用于指定生成的视频分辨率档位。仅用于调整视频的清晰度（总像素），不改变视频的宽高比，**视频宽高比将与输入图像 img_url 的宽高比保持一致**。
      ...
      - wan2.2-i2v：可选值：480P、720P。默认值为`720P`。

  - **duration** `*integer*` （可选）

    - 生成视频的时长，单位为秒。该参数的取值依赖于 model参数：
      - wan2.2-i2v：固定为5秒，且不支持修改。
...
metadata:
  ...
  labels:
    kueue.x-k8s.io/queue-name: "{{ queue_name }}" # <-- 动态渲染：tenant-high-priority-queue 或 tenant-low-priority-queue
spec:
  containers:
  - name: wan22
    args:
...
sequenceDiagram
    ...
    Controller->>Controller: 渲染 PyTorchJob YAML, <br/>添加 'queue: tenant-high-priority-queue', 'priority: 0'
